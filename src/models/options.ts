...
import {isOfType, arrayMoveItem, isType, Type, uid, propPath, countDecimals, roundDecimals} from 'lib/utils';
...
function validateNumber(value: unknown, options?: ValidateNumberOptions): number {
	...
	if (options?.step != null) {
		...
		const decimalEliminator = Math.pow(10, countDecimals(options.step));
		const remainder = (number * decimalEliminator) % (options.step * decimalEliminator);
		if (remainder > 0) {
